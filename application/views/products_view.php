...
<div style="margin:0px auto; width:600px;" >
	...
	<table border="0" cellpadding="5px" cellspacing="1px" style="font-family:Verdana, Geneva, sans-serif; color:#000; border:10px solid #E1E1E1; margin:10px; font-size:12px; background-color:#E1E1E1" width="100%">
		...
		<?php
		echo form_open('products_view/update_cart');
		$grand_total = 0; $i = 1;
		
		foreach ($cart as $item):
			echo form_hidden('products_view['. $item['id'] .'][id]', $item['id']);
			echo form_hidden('products_view['. $item['id'] .'][rowid]', $item['rowid']);
			echo form_hidden('products_view['. $item['id'] .'][name]', $item['name']);
			echo form_hidden('products_viewt['. $item['id'] .'][price]', $item['price']);
			echo form_hidden('products_view['. $item['id'] .'][picture]', $item['qty']);
		?>
		<tr bgcolor="#FFFFFF">
			<td>
			</td>
			...
			<td>
				<?php echo anchor('products_view/remove/'.$item['rowid'],'Cancel'); ?>
			</td>
			<?php endforeach; ?>
		</tr>
		<tr>
